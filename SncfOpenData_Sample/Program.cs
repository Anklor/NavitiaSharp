...
namespace SncfOpenData
{
    class Program
    {
        ...
        {
            List<StopArea> allStopAreas = GetAllResults<StopArea>(api, "stop_areas");
            var json = JsonConvert.SerializeObject(allStopAreas, Formatting.Indented);
        ...
        static List<T> GetAllResults<T>(SncfApi sncfApi,string resourcePath, int chunckSize = 1000) where T : new()
        {
            ...
            while (hasData)
            {
                ...
                else
                {
                    ...
                    int maxPage = (int)Math.Floor((float)pagedResult.Pagination.TotalResult / chunckSize);
